...
| Quality admission tooling | `reference/image/qwen/evaluate_quality_suite.py` validates the frozen suite and fixtures, emits deterministic per-tier plans, requires exact evaluator identity and logical model pairing, hash-checks every unique 1024px PNG, rejects incomplete/non-finite/blank/uniform evidence, checks BF16 and candidate absolute floors, computes the three paired one-sided bootstrap gates with the frozen PCG64 policy, and enforces blinded three-rater Wilson gates over at least 200 stratified pairs plus all 50 identity pairs. `xrt image quality-corpus` loads one exact bundle once, produces a deterministic role/side shard, writes atomic PNG/checkpoint hashes, and resumes only matching plan/bundle outputs. Dedicated corpus and metric workflows require the isolated runner label/machine marker, exact dispatch confirmations, and at least 24,000 MiB VRAM. `run_quality_evaluators.py` binds every checkpoint to the plan and PNG digest, loads OpenCLIP 3.3.0 from the immutable local snapshot, computes DINOv2 structural and fixed-crop face-region identity, runs the exact PaddleOCR-VL v1.6 recognition snapshot without an unpinned layout detector, and exports the complete compiler schema. `prepare_quality_review.py` verifies all 600 paired outputs, selects exactly 200 stratified pairs including all 50 identity pairs, balances candidate A/B slots 100/100, emits three blinded offline reviewer forms, and compiles exactly 600 pseudonymous ratings. | The corpus, metric, review, and admission producers have passed synthetic and fail-closed validation only; the real model/evaluator workloads have not run. No actual 1024x1024 BF16/candidate corpus, metric export, or human ratings have been supplied, so no tier's quality gate is closed. |
