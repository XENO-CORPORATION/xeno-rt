...
| Quality admission tooling | `reference/image/qwen/evaluate_quality_suite.py` validates the frozen suite and fixtures, emits deterministic per-tier plans, requires exact evaluator identity and logical model pairing, hash-checks every unique 1024px PNG, rejects incomplete/non-finite/blank/uniform evidence, checks BF16 and candidate absolute floors, computes the three paired one-sided bootstrap gates with the frozen PCG64 policy, and enforces blinded three-rater Wilson gates over at least 200 stratified pairs plus all 50 identity pairs. `xrt image quality-corpus` now loads one exact bundle once, produces a deterministic role/side shard, writes atomic PNG/checkpoint hashes, and resumes only matching plan/bundle outputs. `.github/workflows/image-quality-corpus.yml` requires a dedicated-runner label, a machine marker, an exact dispatch confirmation, and at least 24,000 MiB VRAM. `prepare_quality_review.py` verifies all 600 paired outputs, selects exactly 200 stratified pairs including all 50 identity pairs, balances candidate A/B slots 100/100, emits three blinded offline reviewer forms, and compiles exactly 600 pseudonymous ratings into the admission schema. | The admission compiler and review compiler have passed synthetic and fail-closed validation only. The pinned CLIP/DINO/OCR metric-export producer remains to be implemented and executed. No actual 1024x1024 BF16/candidate corpus, metric export, or human ratings have been supplied, so no tier's quality gate is closed. |
