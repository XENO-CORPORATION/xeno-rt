...
- Real Gemma4 sequential top-token parity passes through position 3, but a non-winning position-3 logit has a `9.0442` CPU/CUDA delta. Layerwise localization or an agreed semantic/top-k tolerance is still required before claiming strict full-vector parity for longer Gemma4 sequences.
