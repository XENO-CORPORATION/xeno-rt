...
- CUDA KV cache modes use device page tables and GPU-side logical-to-physical addressing. Reusable prefixes are retained by a bounded runtime cache as immutable snapshots. Low-level shared F32, Q8, KQ4/VQ8, and adaptive hot-F32/cold-KQ4-VQ8 caches prove physical-page reuse, prefix sharing, partial-page copy-on-write, direct page-table attention, explicit cross-stream ordering, and graph-bound page/table/route lifetimes with stale replay rejection. Runtime snapshots for all four modes now convert once into width-grouped shared page pools; cache hits attach those pages and copy only a touched partial page. Q8 and KQ4/VQ8 import preserves logical source-page order and copies compressed payloads plus scales device-to-device without dequantizing or requantizing. Agent-adaptive import also preserves already-cold compressed rows, gathers hot rows device-to-device, keeps a stable route table, and supports steady-state hot-to-cold migration without mutating the retained snapshot. Cold-to-hot migration and route patterns that would reorder retained cold rows are rejected explicitly.
- Runtime shared-page arenas are preallocated per KV width and bounded. F32/Q8/KQ4 arenas cover one retained snapshot plus one fully diverged attached session. Adaptive hot arenas reserve two full page capacities per layer so a hot tier can be rebuilt while its retained snapshot remains live; adaptive cold arenas reserve one full capacity plus one divergence page because retained compressed rows remain shared. Additional concurrent writers can exhaust a pool and return an explicit CUDA error; expanding this concurrency contract is pending. Model, scratch, KV, page-table, route, graph-parameter, and temporary CUDA slices otherwise allocate through the device's current stream-ordered CUDA memory pool when supported.
- Sampled device-wide VRAM usage, process peak host residency, a clone-shared ledger for explicit xeno-owned CUDA allocations, and CUDA pool used/reserved current/peak telemetry are wired. The explicit ledger records category-owned current/peak bytes, allocation calls, and total allocated bytes, including temporary old-plus-new overlap during KV growth. Pool telemetry adds driver-level backing-allocation visibility but does not attribute bytes by xeno category. Batch-1 CUDA Graph replay is wired for ordinary contiguous F32 and runtime-attached shared F32/Q8/KQ4-VQ8/agent-adaptive standard-dense KV. Shared graphs retain and validate their page pools, page tables, page leases, and adaptive route table for the executable lifetime, preallocate a stable private suffix before capture, and reject stale topology or newly shared writable pages before replay. Agent-adaptive capture is deliberately bounded to homogeneous shared layers whose entire prepared decode suffix remains in the final policy recent-window hot tier; route-changing horizons fall back to eager CUDA. Gemma4, contiguous quantized-KV graphs, shared-page concurrent parent-graph composition, and larger batch graph variants also retain eager paths where no stable graph contract exists.
...
- 2026-07-13: CUDA validation run `29223964107` was cancelled after the self-hosted runner exhausted C: before checkout; it is infrastructure failure evidence, not a CUDA regression. The Windows CUDA job now pins `CARGO_TARGET_DIR` below `X:\actions-runner-cache\xeno-rt`, rejects paths outside that root, and requires at least `64 GiB` free on X:. Exact-head replacement run `29226250864` reports `2,170.4 GiB` free on X:, passes PTX reproducibility, the complete serial model-free RTX parity gate, shared-page pointer attention, cleanup, and the safe CUDA check. Real-model and server smokes were not requested in that run.
- 2026-07-13: Shared F32 KV access now carries explicit CUDA events between cudarc execution streams and the default stream. Append and pointer-table attention expose scheduler-stream entry points, while allocation, partial-page copy-on-write, pointer refresh, truncate, clear, trim, and drop preserve ordering before pages can be reused or freed. The first correctness policy is deliberately conservative: one pool-wide completion fence serializes shared-page submissions across streams until reader concurrency is optimized separately. Exact-head model-free RTX run `29276181776` passes the dedicated producer/consumer handoff test across a page boundary, stream-ordered partial-page copy-on-write, snapshot isolation, clear, immediate physical-page reuse, PTX reproducibility, the complete serial parity gate, cleanup, and the safe CUDA check. Runtime adoption is now gated on CUDA Graph pointer-lifetime validation and quantized shared pools rather than missing stream ordering.
- 2026-07-13: Shared F32 pointer-table attention can now be captured in a cache-bound CUDA Graph wrapper. The wrapper retains the page-table allocation and every captured page lease, binds replay to the originating cache, topology epoch, and cache length, rejects stale replay after page growth, copy-on-write, clear, or truncate, and returns an event for default/custom-stream completion. Query and output allocations remain caller-owned and must outlive the graph and completion event. Exact-head model-free RTX run `29278511863` passes graph replay parity, stale-topology rejection after append and truncate, retained-page lifetime/release checks, PTX reproducibility, the complete serial parity gate, cleanup, and the safe CUDA check while using `X:\actions-runner-cache\xeno-rt\cargo-target\cuda` with `2,116.64 GiB` free. Runtime adoption is now gated on shared quantized page pools and session integration rather than missing graph pointer-lifetime rules.
- 2026-07-13: Added the first shared quantized allocator as `CudaQ8KvPagePool`/`CudaSharedQ8LayerKvCache`. One bounded preallocated arena owns Q8 key/value bytes plus per-row F32 scales; caches hold reusable physical-page leases and stable logical-to-physical `u32` page tables. Prefix snapshots share leases, partial-page writes copy quant bytes and both scale pages device-to-device before remapping, and the existing Q8 append/dequantize/direct-attention kernels consume the shared page map without a second quantized kernel family. Exact-head model-free RTX run `29280533531` passes source isolation, cross-page attention against the scalar reference, page return/reuse, PTX reproducibility, the complete serial parity gate, cleanup, and the safe CUDA check on commit `600ad10` while using `X:\actions-runner-cache\xeno-rt\cargo-target\cuda` with `2,113.18 GiB` free. Real-model and server smokes were deliberately disabled. Q8 cross-stream/graph support, KQ4/VQ8 and adaptive shared pools, and runtime session integration remain pending.
- 2026-07-13: Shared Q8 pages now use the same conservative pool-wide CUDA event handoff as shared F32 pages and expose default/custom-stream append and attention entry points. Cache-bound Q8 attention graphs retain the arena, logical page table, and captured physical-page leases; replay is bound to the originating cache topology epoch and length and rejects stale use after growth, copy-on-write, truncate, or clear. Exact-head model-free RTX run `29282304779` passes `shared_q8_kv_cross_stream_handoff_preserves_cow_and_reuse` and `shared_q8_kv_attention_graph_retains_pages_and_rejects_stale_topology`, PTX reproducibility, the complete serial parity gate, cleanup, and the safe CUDA check on commit `025a38b` while using `X:\actions-runner-cache\xeno-rt\cargo-target\cuda` with `2,107.9 GiB` free. Real-model and server smokes were deliberately disabled. KQ4/VQ8 and adaptive shared pools plus runtime session integration remain pending.
- 2026-07-13: Added `CudaKq4Vq8KvPagePool`/`CudaSharedKq4Vq8LayerKvCache` with a bounded arena for packed Q4 keys, Q8 values, per-64-element F32 key scales, and per-row F32 value scales. It preserves the established 64-element key-scale contract, stable logical-to-physical page maps, prefix lease sharing, partial-page copy-on-write, conservative cross-stream event handoff, direct online attention, and cache-bound graph lifetime/staleness checks. Exact-head model-free RTX run `29283919387` passes allocator/reuse/COW parity, producer/consumer stream parity, and graph lifetime/stale replay parity at 128-wide heads, forcing two independent key-scale groups per row. The same run passes PTX reproducibility, the complete serial parity gate, cleanup, and the safe CUDA check on commit `8594db9` while using `X:\actions-runner-cache\xeno-rt\cargo-target\cuda` with `2,103.65 GiB` free. Real-model and server smokes were deliberately disabled. Adaptive shared composition and runtime session integration remain pending.
- 2026-07-13: Added `CudaSharedAdaptiveLayerKvCache`, composing the validated shared F32 and KQ4/VQ8 pools behind a stable GPU route table. The composite supports mixed hot/cold append, immutable prefix snapshots, independent partial-page copy-on-write in both tiers, fixed-order default/custom-stream fencing across both pools and routes, direct mixed attention over F32 page pointers plus the KQ4/VQ8 arena, and cache-bound graph ownership/stale replay rejection. Exact-head model-free RTX run `29286340799` passes both-tier prefix/COW reuse, alternating producer/consumer stream attention, and graph route/page lifetime parity at width 128 on commit `750c243`. The same run passes the hosted PTX job, complete serial safe gate, cleanup, and `safe CUDA check passed` on an NVIDIA GeForce RTX 4090 using `X:\actions-runner-cache\xeno-rt\cargo-target\cuda` with `2,101.08 GiB` free. Real-model, SafeTensors, and server smokes were deliberately disabled. Runtime session integration remains pending.
- 2026-07-13: Integrated shared F32 pages at the runtime prefix boundary. F32 cache misses and their batch-1 graph path remain on contiguous storage; snapshot insertion performs device-side row transfer into lazy width-grouped page pools, and a cache hit materializes page-sharing views instead of cloning the full layer allocation. Shared-hit append and attention use the pointer-table path, a partial-page first write becomes private while the immutable prefix remains unchanged, and runtime graph status explicitly reports eager fallback until whole-decode shared graph capture is wired. Exact-head model-free RTX run `29288190041` passes `shared_f32_projected_bytes_count_live_pages_and_stable_tables`, `cuda_runtime_shared_f32_prefix_attachment_copies_only_touched_page`, hosted PTX compilation, the complete serial safe gate, cleanup, and `safe CUDA check passed` on commit `6ef3630` using an NVIDIA GeForce RTX 4090 and `X:\actions-runner-cache\xeno-rt\cargo-target\cuda` with `2,100.25 GiB` free. Real-model and server smokes were deliberately disabled. Q8, KQ4/VQ8, and adaptive runtime snapshot integration remains pending.
- 2026-07-16: Integrated runtime shared snapshots for Q8 and KQ4/VQ8. Snapshot insertion reads only the small logical source page map on the host, then copies live compressed key/value payloads and F32 scales device-to-device into bounded width-grouped arenas; KQ4 preserves the established 64-element scale groups. Cache hits attach shared leases, partial final-page writes copy on write, immutable snapshots remain unchanged, and direct shared quantized attention is used in standard dense and Gemma4 windowed paths. Runtime shared quantized sessions deliberately report eager graph fallback. Exact-head model-free RTX run `29473136040` passes `shared_quantized_projected_bytes_count_live_pages_and_stable_tables`, `shared_quantized_prefix_import_preserves_remapped_rows_and_partial_page_cow`, and `cuda_runtime_shared_quantized_prefix_attachment_preserves_rows_and_cow`, plus hosted PTX reproducibility, the complete serial safe gate, cleanup, and the 165-second quiet-exit soak on commit `897ae07`. The runner was an NVIDIA GeForce RTX 4090 using `X:\actions-runner-cache\xeno-rt\cargo-target\cuda` with `2,077.16 GiB` free. Real-model, SafeTensors, diagnostics, CPU comparison, and server smokes were deliberately disabled; adaptive composite runtime snapshots and shared-page whole-decode graph capture remain pending.
- 2026-07-16: Integrated runtime shared snapshots for agent-adaptive KV. Snapshot conversion builds width-grouped hot-F32 and cold-KQ4/VQ8 arenas, imports already-cold compressed payloads/scales device-to-device, gathers hot rows device-to-device, and retains a stable GPU route table. Attached sessions preserve compressed cold rows and migrate only aged hot rows into cold storage; cold-to-hot and cold-row-reordering transitions fail explicitly. Hot pools reserve two page capacities per layer for snapshot-safe rebuilds, while cold pools reserve one capacity plus one copy-on-write page. Exact-head model-free RTX run `29475242810` passes `shared_adaptive_bytes_cover_both_tiers_routes_and_hot_rebuild_headroom`, `shared_adaptive_prefix_import_migrates_hot_rows_without_mutating_snapshot`, and `cuda_runtime_shared_adaptive_prefix_migrates_aged_rows_and_preserves_snapshot`, plus hosted PTX reproducibility, the complete serial safe gate, cleanup, and the quiet-exit soak on commit `1e02f71` (implementation base `518e353`). The runner was an NVIDIA GeForce RTX 4090 using `X:\actions-runner-cache\xeno-rt\cargo-target\cuda` with `2,071.71 GiB` free. Real-model, SafeTensors, packed-format, diagnostics, CPU comparison, benchmark, and server jobs were deliberately disabled. Shared-page whole-decode graph capture and broader concurrent-writer admission remain pending.
- 2026-07-16: Extended batch-1 whole-decode CUDA Graph replay to runtime-attached shared F32 prefix sessions. Shared append and direct attention kernels read position, cache length, and attention start from the resident decode-parameter buffer; capture first copy-on-writes a shared partial page and preallocates a bounded private suffix so page-table topology remains stable. The executable retains the originating page pool, page table, and page leases, validates their identities and topology before replay, rejects a writable page that gained another owner, and is proactively invalidated before snapshot, clear, or truncate. A shared-KV discriminator prevents graph-cache collisions with contiguous layouts, while shared F32 sessions remain excluded from concurrent parent-graph composition until that ownership contract is implemented. Exact-head model-free RTX run `29477157867` passes `shared_f32_decode_graph_replays_dynamic_append_and_attention`, `cuda_runtime_shared_f32_prefix_attachment_copies_only_touched_page`, and `cuda_repeated_prompt_reuses_immutable_prefix_kv`, plus hosted PTX reproducibility, the complete serial safe gate, and cleanup on commit `2c4a078`. The runner was an NVIDIA GeForce RTX 4090 using `X:\actions-runner-cache\xeno-rt\cargo-target\cuda`; initial telemetry reported `8,678 MiB / 24,564 MiB`, `42 C`, and 4% utilization with `2,069.18 GiB` free on X:. Real-model, SafeTensors fixture, packed-format, diagnostics, CPU comparison, benchmark, and server jobs were deliberately disabled for runner safety. Shared Q8/KQ4-VQ8/adaptive whole-decode graphs and shared-F32 concurrent parent-graph composition remain pending.
- 2026-07-16: Extended the same batch-1 whole-decode graph contract to runtime-attached shared Q8 prefixes. Q8 append and online-attention kernels consume the resident position, cache length, and attention-start parameters; capture preallocates a private suffix and retains a `CudaSharedQ8GraphBinding` over the pool arena, page table, page leases, topology epoch, and first writable page. Replay validates cache/pool identity, exact page topology, prepared append position, and exclusive writable-page ownership, while snapshot, truncate, clear, and topology changes invalidate the executable. Exact-head model-free RTX run `29478977926` passes `shared_q8_decode_graph_replays_dynamic_append_and_attention`, `cuda_runtime_shared_quantized_prefix_attachment_preserves_rows_and_cow`, and `cuda_repeated_prompt_reuses_immutable_prefix_kv`, plus hosted PTX reproducibility and the complete serial safe gate on commit `68d56b3`. The runner was an NVIDIA GeForce RTX 4090 using `X:\actions-runner-cache\xeno-rt\cargo-target\cuda` with `2,068.47 GiB` free; initial telemetry reported `8,953 MiB / 24,564 MiB`, `44 C`, and 4% utilization. Real packed fixtures, real-model smoke, concurrent server smoke, diagnostics, CPU comparison, and benchmark work were deliberately disabled, so this run is focused synthetic/runtime parity evidence rather than a throughput claim. Shared KQ4/VQ8 and adaptive whole-decode graphs plus shared F32/Q8 concurrent parent-graph composition remain pending.
- 2026-07-16: Extended batch-1 whole-decode graph replay to runtime-attached shared KQ4/VQ8 prefixes while preserving packed Q4 keys, Q8 values, and the established 64-element key-scale groups. Append and online-attention kernels consume resident decode parameters, and `CudaSharedKq4Vq8GraphBinding` retains the arena, page table, compressed page leases, topology epoch, and first writable page. Capture copy-on-writes a shared partial page and preallocates a bounded private suffix; replay rejects changed page identities, stale topology, unprepared append positions, and writable pages that gain another owner. Exact-head model-free RTX run `29480747566` passes `shared_kq4_vq8_decode_graph_replays_dynamic_append_and_attention`, `cuda_runtime_shared_quantized_prefix_attachment_preserves_rows_and_cow`, and `cuda_repeated_prompt_reuses_immutable_prefix_kv`, plus hosted PTX reproducibility and the complete serial safe gate on commit `905d652`. The runner was an NVIDIA GeForce RTX 4090 using `X:\actions-runner-cache\xeno-rt\cargo-target\cuda` with `2,068.43 GiB` free; initial telemetry reported `8,721 MiB / 24,564 MiB`, `42 C`, and 5% utilization. Real packed fixtures, real-model smoke, concurrent server smoke, diagnostics, CPU comparison, and benchmark work were deliberately disabled, so this run adds parity/lifetime evidence rather than throughput data. Shared adaptive whole-decode graphs plus shared F32/Q8/KQ4-VQ8 concurrent parent-graph composition remain pending.
- 2026-07-16: Extended batch-1 whole-decode graph replay to runtime-attached shared agent-adaptive prefixes. The runtime admits capture only when every prepared suffix position remains in the final recent-window hot tier, then preallocates hot-page capacity and retains hot/cold pools, both page maps, all captured leases, the route table, topology epochs, and the first writable hot page. Replay validates the complete mixed-tier identity/topology contract; unsupported route horizons, stale topology, or changed ownership use explicit eager fallback. Model-free RTX run `29484055092` passes `cuda_adaptive_graph_requires_entire_suffix_in_final_hot_window`, `shared_adaptive_decode_graph_replays_hot_suffix_and_mixed_attention`, and `cuda_runtime_shared_adaptive_prefix_migrates_aged_rows_and_preserves_snapshot`, plus PTX reproducibility and `safe CUDA check passed` on commit `1771aaf`. Real-model, SafeTensors, packed-format, diagnostics, CPU comparison, benchmark, and server jobs were deliberately disabled, so this is graph correctness/lifetime evidence rather than throughput data.
- 2026-07-16: Restored standard hosted CI after removing the private sibling `xeno-lib` build dependency from `xrt-vision`; the active BiRefNet preprocessing, ONNX Runtime inference, and postprocessing pipeline now lives in `xrt-vision` with CPU execution-provider fallback and optional CUDA registration. CI also exposed a real process-crash defect in the global CPU `SpinPool`: concurrent callers could replace its single raw job pointer while workers still consumed the prior job. Dispatch is now mutex-serialized and covered by a four-caller regression test. Hosted run `29517196817` passes Check, Format, Clippy, Test, and Benchmarks Compile. Guarded RTX revalidation run `29517359954` passes hosted PTX compilation, the complete serial CUDA parity gate, the three adaptive graph tests above, cleanup, and `safe CUDA check passed` at commit `c19f690`; initial RTX 4090 telemetry was `8,501 MiB / 24,564 MiB`, so all real-model, SafeTensors, server, CPU-comparison, and benchmark jobs remained disabled under the `4,096 MiB` safety threshold.
...
- A separate low-level `CudaF32KvPagePool`/`CudaSharedF32LayerKvCache` path validates lazy bounded page allocation, stable pointer tables, reusable page leases, prefix sharing, partial-page copy-on-write, device-pointer append/gather, direct online attention, explicit cross-stream event handoff, and cache-bound CUDA Graph ownership. Model-free RTX runs `29206337572`, `29207272334`, `29208223785`, `29276181776`, and `29278511863` cover allocator ownership, cross-page access, GQA, attention windows, copy-on-write isolation, producer/consumer stream ordering, safe page reuse, graph-retained table/page lifetimes, and stale replay rejection.
- `CudaQ8KvPagePool`/`CudaSharedQ8LayerKvCache` adds the first shared quantized path. It preallocates a bounded split Q8 arena, leases physical pages through shared ownership, keeps a stable device page map per cache, copies only a touched shared page on write, and reuses the existing Q8 append/dequantize/online-attention kernels. Model-free RTX runs `29280533531` and `29282304779` validate prefix isolation, page-boundary attention, released-page reuse, explicit cross-stream producer/consumer ordering, cache-bound graph lifetime retention, and stale-topology rejection.
- `CudaKq4Vq8KvPagePool`/`CudaSharedKq4Vq8LayerKvCache` extends the same ownership, copy-on-write, stream-ordering, and graph-lifetime contract to packed Q4 keys plus Q8 values without changing the 64-element key-scale grouping. Model-free RTX run `29283919387` validates the allocator and direct attention path at 128-wide heads.
- `CudaSharedAdaptiveLayerKvCache` composes shared hot F32 pages, shared cold KQ4/VQ8 pages, and a retained GPU route table without duplicating either storage tier. Model-free RTX run `29286340799` validates both-tier COW, mixed attention across streams, and graph route/page lifetime at 128-wide heads.
- Runtime F32, Q8, KQ4/VQ8, and agent-adaptive prefix snapshots use shared page representations on insertion, and cache hits attach those pages copy-on-write. Quantized imports remain compressed. Adaptive imports preserve cold rows, copy hot rows device-to-device, and support bounded hot-to-cold route migration while retaining immutable snapshots. Source miss sessions remain contiguous so their established CUDA Graph decode path does not regress. Shared F32, Q8, KQ4/VQ8, and eligible agent-adaptive cache hits now use validated batch-1 whole-decode graph replay with stable private suffix capacity and retained page/table ownership. Adaptive capture requires the complete prepared suffix to remain hot under the final policy horizon; other route patterns retain explicit eager fallback. Model-free RTX runs `29288190041`, `29473136040`, `29475242810`, `29477157867`, `29478977926`, `29480747566`, `29484055092`, and `29517359954` validate the runtime boundary and all four shared-KV graph variants.
...
- Complete and RTX 4090 validated for the first target: standard dense batch-1 decode with contiguous F32 or runtime-shared F32/Q8/KQ4-VQ8/agent-adaptive paged KV and resident F32/F16/BF16/Q8_0/Q4_0/Q4_K/Q5_K/Q6_K weight layouts.
...
- Runtime-attached shared F32 prefixes preallocate a bounded private suffix, retain page/table/pool ownership with the executable, and reject stale topology or externally shared writable pages before replay. Model-free RTX run `29477157867` validates dynamic append/attention parameters, runtime copy-on-write isolation, repeated-prompt parity, and graph lifetime on commit `2c4a078`.
- Runtime-attached shared Q8 prefixes use the same bounded private-suffix and executable-owned page/table/pool lifetime contract without dequantizing retained KV. Model-free RTX run `29478977926` validates dynamic Q8 append/attention parameters, runtime copy-on-write isolation, repeated-prompt parity, and stale-lifetime rejection on commit `68d56b3`.
- Runtime-attached shared KQ4/VQ8 prefixes retain compressed page storage and the 64-element key-scale contract while using the same bounded private-suffix and stale-lifetime checks. Model-free RTX run `29480747566` validates dynamic KQ4/VQ8 append/attention parameters, runtime copy-on-write isolation, repeated-prompt parity, and graph ownership on commit `905d652`.
- Runtime-attached shared agent-adaptive prefixes capture only a bounded suffix that stays entirely in the final hot tier. The executable retains mixed hot/cold storage and routes, validates both tiers before replay, and falls back eagerly when the requested horizon would change routing. Runs `29484055092` and `29517359954` validate the policy gate, mixed-attention replay, stale-state rejection, snapshot migration, and safe completion.
- Gemma4 variable-width layers, route-changing adaptive graph horizons, shared-page concurrent parent-graph composition, contiguous quantized-KV graphs, and graph batch sizes above one remain eager CUDA extensions; they do not block the validated Phase 5 batch-1 standard-dense target.
...
- CUDA F32, Q8, KQ4/VQ8, and agent-adaptive snapshots use runtime shared page pools. Attach shares page leases; first mutation copies only a shared partial page device-to-device. Quantized snapshot import preserves compressed payloads and scales exactly. Adaptive snapshots compose hot F32 pages, cold KQ4/VQ8 pages, and route tables; steady-state migration moves only hot rows to cold storage and leaves retained snapshots immutable.
- Runtime shared page arenas are width-grouped and bounded. F32/Q8/KQ4 pools reserve one retained snapshot plus one fully diverged attached session. Adaptive hot pools reserve two page capacities per layer for rebuild overlap, and adaptive cold pools reserve one capacity plus one divergence page. Pool exhaustion is explicit, and retained arena plus stable page/pointer/route-table bytes count against prefix/scheduler KV accounting.
...
- Model-free RTX run `29473136040` proves runtime Q8 and KQ4/VQ8 snapshot import, remapped source-page ordering, compressed device-to-device transfer, partial-page copy-on-write, immutable-source isolation, and shared-hit attention on an RTX 4090.
- Model-free RTX run `29475242810` proves runtime agent-adaptive snapshot conversion, mixed-tier route preservation, cold compressed-row reuse, hot-to-cold migration, immutable-snapshot isolation, bounded arena accounting, and later append copy-on-write on an RTX 4090.
- Model-free RTX run `29477157867` proves batch-1 whole-decode graph replay for runtime-attached shared F32 prefixes, including dynamic decode parameters, bounded private-suffix preparation, retained page/table ownership, immutable-prefix isolation, and stale replay rejection on an RTX 4090.
- Model-free RTX run `29478977926` proves batch-1 whole-decode graph replay for runtime-attached shared Q8 prefixes, including dynamic append/attention parameters, bounded private-suffix preparation, retained compressed page/table ownership, copy-on-write isolation, and stale topology/lifetime rejection on an RTX 4090.
- Model-free RTX run `29480747566` proves batch-1 whole-decode graph replay for runtime-attached shared KQ4/VQ8 prefixes, including dynamic append/attention parameters, bounded private-suffix preparation, retained compressed page/table ownership, the 64-element key-scale contract, copy-on-write isolation, and stale topology/lifetime rejection on an RTX 4090.
- Model-free RTX runs `29484055092` and `29517359954` prove bounded batch-1 whole-decode graph replay for runtime-attached shared agent-adaptive prefixes, including the all-hot final-suffix policy gate, retained hot/cold pools and route table, mixed attention, immutable snapshot migration, and stale topology/ownership rejection on an RTX 4090.
...
- Broaden shared agent-adaptive graph capture beyond the current all-hot suffix horizon when route-changing migration can preserve stable graph bindings, then admit all shared F32/Q8/KQ4-VQ8/adaptive sessions into concurrent parent-graph composition with equivalent ownership and stale-replay checks.
- Expand shared-page arena sizing/admission beyond one retained snapshot plus one fully diverged attached writer per width, with concurrency stress and scheduler-accounting evidence.
