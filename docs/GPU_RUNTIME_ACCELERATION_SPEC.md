...
- CUDA KV cache modes use device page tables and GPU-side logical-to-physical addressing. Reusable prefixes are retained by a bounded runtime cache as immutable snapshots. Low-level shared F32, Q8, KQ4/VQ8, and adaptive hot-F32/cold-KQ4-VQ8 caches prove physical-page reuse, prefix sharing, partial-page copy-on-write, direct page-table attention, explicit cross-stream ordering, and graph-bound page/table/route lifetimes with stale replay rejection. Runtime F32 prefix snapshots now convert once into width-grouped shared page pools; cache hits attach those pages and copy only a touched partial page. Ordinary F32 misses retain the established contiguous CUDA Graph path. Q8, KQ4/VQ8, and adaptive runtime snapshots still use contiguous full-allocation clones.
- Model, scratch, KV, page-table, route, graph-parameter, and temporary CUDA slices allocate through the device's current stream-ordered CUDA memory pool when the device supports memory pools. Scratch ownership remains session-local; there is no category-specific user-space scratch arena, and runtime shared-page allocation is currently limited to F32 prefix snapshots.
...
- Extend the validated runtime F32 shared-snapshot contract to Q8, KQ4/VQ8, and adaptive composite snapshots so every cache mode copies only touched pages instead of the complete session allocation.
