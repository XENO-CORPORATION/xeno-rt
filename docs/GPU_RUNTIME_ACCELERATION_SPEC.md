...
- CUDA KV cache modes use device page tables and GPU-side logical-to-physical addressing. Reusable prefixes are retained by a bounded runtime cache as immutable snapshots. Low-level shared F32, Q8, KQ4/VQ8, and adaptive hot-F32/cold-KQ4-VQ8 caches now prove physical-page reuse, prefix sharing, partial-page copy-on-write, direct page-table attention, explicit cross-stream ordering, and graph-bound page/table/route lifetimes with stale replay rejection. Runtime sessions still use per-layer contiguous allocations; integration of these validated shared pools into session snapshots remains future memory-efficiency work.
...
- `CudaSharedAdaptiveLayerKvCache` composes shared hot F32 pages, shared cold KQ4/VQ8 pages, and a retained GPU route table without duplicating either storage tier. Model-free RTX run `29286340799` validates both-tier COW, mixed attention across streams, and graph route/page lifetime at 128-wide heads. Runtime decode still uses the established contiguous cache representation; session integration remains pending.
...
- Integrate the validated low-level F32, Q8, KQ4/VQ8, and adaptive composite page pools with runtime snapshots so first write copies only touched pages instead of the complete session allocation.
