...
Status: Draft implementation spec, Phase 8 dense Qwen2 and AutoAWQ GEMM4 CUDA validated; GPTQ/compressed formats pending
Date: 2026-06-19
...
- Explicitly pending: GPTQ, compressed-tensors, other AWQ packing/execution versions, EXL3 evaluation, SafeTensors CPU decode, and architecture support beyond Qwen2.
- Safety: SafeTensors model directories require a CUDA-enabled build and `cuda` or `auto`; requesting CPU returns a clear unsupported error. Unsupported quantization methods and packing versions are rejected before upload and are never reinterpreted as AutoAWQ GEMM.
