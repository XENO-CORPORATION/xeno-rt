...
- CUDA KV cache modes use device page tables and GPU-side logical-to-physical addressing. Reusable prefixes are retained by a bounded runtime cache as immutable snapshots. Low-level shared F32 and Q8 allocators now prove physical-page reuse, prefix sharing, partial-page copy-on-write, direct page-table attention, explicit cross-stream ordering, and graph-bound page/table lifetimes with stale replay rejection. Runtime sessions still use per-layer contiguous allocations; KQ4/VQ8 and agent-adaptive shared pools plus runtime integration remain future memory-efficiency work.
...
- `CudaQ8KvPagePool`/`CudaSharedQ8LayerKvCache` adds the first shared quantized path. It preallocates a bounded split Q8 arena, leases physical pages through shared ownership, keeps a stable device page map per cache, copies only a touched shared page on write, and reuses the existing Q8 append/dequantize/online-attention kernels. Model-free RTX runs `29280533531` and `29282304779` validate prefix isolation, page-boundary attention, released-page reuse, explicit cross-stream producer/consumer ordering, cache-bound graph lifetime retention, and stale-topology rejection. Runtime decode still uses the established contiguous cache representation; KQ4/VQ8 and agent-adaptive shared pools plus session integration remain pending.
...
- Integrate the validated low-level F32 and Q8 page pools with runtime snapshots so first write copies only touched pages instead of the complete session allocation; extend the same ownership, stream-ordering, and graph-lifetime contract to KQ4/VQ8 and agent-adaptive storage.
