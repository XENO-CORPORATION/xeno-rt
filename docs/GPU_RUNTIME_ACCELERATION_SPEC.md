...
- Runtime F32 prefix snapshots use the shared page representation on insertion and cache hits attach shared pages copy-on-write. The source miss session remains contiguous so its established CUDA Graph decode path does not regress. Shared-hit sessions deliberately use eager CUDA until full-decode graph ownership is extended to the runtime shared cache variant. Q8, KQ4/VQ8, and adaptive runtime snapshots remain on the established contiguous representation.
...
- CUDA F32, Q8, and KQ4/VQ8 snapshots use runtime shared page pools. Attach shares page leases; first mutation copies only a shared partial page device-to-device. Quantized snapshot import preserves compressed payloads and scales exactly, while adaptive composite snapshots retain the older full-allocation clone path until their runtime pool integration is complete.
- Runtime shared page arenas reserve enough pages for one retained snapshot and one fully diverged attached session per width. Pool exhaustion is explicit, and retained arena plus stable page-table bytes count against prefix/scheduler KV accounting.
...
- Extend the validated runtime F32/Q8/KQ4-VQ8 shared-snapshot contract to adaptive composite snapshots so every cache mode copies only touched pages instead of the complete session allocation.
