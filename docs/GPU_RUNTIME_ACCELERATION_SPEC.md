...
- A separate low-level `CudaF32KvPagePool`/`CudaSharedF32LayerKvCache` path now validates lazy bounded page allocation, stable pointer tables, reusable page leases, prefix sharing, partial-page copy-on-write, device-pointer append/gather, direct online attention, and explicit cross-stream event handoff. Model-free RTX runs `29206337572`, `29207272334`, `29208223785`, and `29276181776` cover allocator ownership, cross-page access, GQA, attention windows, copy-on-write isolation, producer/consumer stream ordering, and safe page reuse. Runtime decode still uses the established contiguous cache representation until CUDA Graph pointer-lifetime rules are validated. Q8, KQ4/VQ8, and agent-adaptive shared pools remain pending.
