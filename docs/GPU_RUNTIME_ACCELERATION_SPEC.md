...
- CUDA KV cache modes use device page tables and GPU-side logical-to-physical addressing. Reusable prefixes are retained by a bounded runtime cache as immutable snapshots. A low-level bounded shared F32 page pool now proves physical-page reuse and partial-page copy-on-write, but runtime sessions still use per-layer contiguous allocations; device-pointer attention, cross-stream ordering, quantized pools, and runtime integration remain future memory-efficiency work.
...
- A separate low-level `CudaF32KvPagePool`/`CudaSharedF32LayerKvCache` path now validates lazy bounded page allocation, stable pointer tables, reusable page leases, prefix sharing, and partial-page copy-on-write. Runtime decode still uses the established contiguous cache representation until pointer-table append/attention kernels and stream/graph lifetime rules are validated. Q8, KQ4/VQ8, and agent-adaptive shared pools remain pending.
