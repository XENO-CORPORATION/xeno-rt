...
- CUDA KV cache modes use device page tables and GPU-side logical-to-physical addressing. Reusable prefixes are retained by a bounded runtime cache as immutable snapshots. Low-level shared F32 and Q8 allocators now prove physical-page reuse, prefix sharing, partial-page copy-on-write, and direct page-table attention. The F32 path additionally validates explicit cross-stream ordering and graph-bound page/table lifetimes with stale replay rejection. Runtime sessions still use per-layer contiguous allocations; Q8 stream/graph support, KQ4/VQ8 and agent-adaptive shared pools, and runtime integration remain future memory-efficiency work.
...
- `CudaQ8KvPagePool`/`CudaSharedQ8LayerKvCache` adds the first shared quantized path. It preallocates a bounded split Q8 arena, leases physical pages through shared ownership, keeps a stable device page map per cache, copies only a touched shared page on write, and reuses the existing Q8 append/dequantize/online-attention kernels. Model-free RTX run `29280533531` validates prefix isolation, page-boundary attention, and released-page reuse. Runtime decode still uses the established contiguous cache representation; Q8 stream/graph support, KQ4/VQ8 and agent-adaptive shared pools, and session integration remain pending.
...
- Integrate the validated low-level F32 and Q8 page pools with runtime snapshots so first write copies only touched pages instead of the complete session allocation; extend the same ownership contract to KQ4/VQ8 and agent-adaptive storage.
