...
- CUDA KV cache modes use device page tables and GPU-side logical-to-physical addressing. Reusable prefixes are retained by a bounded runtime cache as immutable snapshots. A low-level bounded shared F32 page pool now proves physical-page reuse, partial-page copy-on-write, device-pointer append/gather, direct pointer-table online attention, explicit cross-stream event ordering, and graph-bound page/table lifetimes with stale replay rejection, but runtime sessions still use per-layer contiguous allocations; quantized shared pools and runtime integration remain future memory-efficiency work.
...
- A separate low-level `CudaF32KvPagePool`/`CudaSharedF32LayerKvCache` path now validates lazy bounded page allocation, stable pointer tables, reusable page leases, prefix sharing, partial-page copy-on-write, device-pointer append/gather, direct online attention, explicit cross-stream event handoff, and cache-bound CUDA Graph ownership. Model-free RTX runs `29206337572`, `29207272334`, `29208223785`, `29276181776`, and `29278511863` cover allocator ownership, cross-page access, GQA, attention windows, copy-on-write isolation, producer/consumer stream ordering, safe page reuse, graph-retained table/page lifetimes, and stale replay rejection. Runtime decode still uses the established contiguous cache representation until the shared allocator is integrated. Q8, KQ4/VQ8, and agent-adaptive shared pools remain pending.
