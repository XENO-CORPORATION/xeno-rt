...
__global__ void Marlin(
    ...
    bool use_fp32_reduce,  // whether to use fp32 global reduce
    int max_shared_mem) {
  // XRT uses a negative max-shared-memory sentinel for its Q5_K path. The
  // upstream parameter is otherwise informational in this kernel. In this
  // mode C0 is a row-major FP32 destination, which avoids a lossy FP16 output
  // round-trip while leaving the upstream ABI and ordinary Marlin paths intact.
  bool xrt_f32_output = max_shared_mem < 0;
  if (xrt_f32_output) max_shared_mem = -max_shared_mem;
  // XRT signals its compact Q5_K resident layout with a negative group
  // count. The ordinary U8 Marlin path is unchanged. Compact Q5_K keeps each
  // sixteen-value Marlin group in 12 bytes (8 low-nibble bytes, a 16-bit high
  // plane, and 2 padding bytes) and expands it only in shared memory.
  bool xrt_packed_q5 = num_groups < 0;
  if (xrt_packed_q5) num_groups = -num_groups;
  ...
  // XRT's optional FP32-output epilogue needs twice the storage of upstream's
  // FP16 reduction layout. Keep it after every upstream shared-memory region
  // so scale and activation reads cannot alias the writeback staging area.
  float* sh_xrt_f32 =
      reinterpret_cast<float*>(sh_a + (stages * a_sh_stage));
  ...
  #pragma unroll
      for (int i = 0; i < (b_sh_wr_iters * b_thread_vecs); i++) {
        ...
        if (xrt_packed_q5) {
          // Four consecutive logical int4 groups occupy 48 aligned bytes.
          // Three lanes fetch those bytes coalesced; the fourth reconstructs
          // its group from lane two. This avoids the 3x transaction inflation
          // of independent 12-byte strided loads.
          const int group_lane = b_gl_idx & 3;
          const int group_base = b_gl_idx - group_lane;
          uint4 packed_line = {0, 0, 0, 0};
          if (group_lane < 3) {
            packed_line = reinterpret_cast<const uint4*>(
                reinterpret_cast<const unsigned char*>(B) + group_base * 12)[group_lane];
          }
          const int warp_group_base = (threadIdx.x & 31) & ~3;
          // Every lane must execute every shuffle named by the full-warp
          // mask. Selecting shuffles inside the four-lane branch is undefined
          // on CUDA and can deadlock the kernel.
          const unsigned int lane0_w =
              __shfl_sync(0xffffffffu, packed_line.w, warp_group_base);
          const unsigned int lane1_z =
              __shfl_sync(0xffffffffu, packed_line.z, warp_group_base + 1);
          const unsigned int lane1_w =
              __shfl_sync(0xffffffffu, packed_line.w, warp_group_base + 1);
          const unsigned int lane2_y =
              __shfl_sync(0xffffffffu, packed_line.y, warp_group_base + 2);
          const unsigned int lane2_z =
              __shfl_sync(0xffffffffu, packed_line.z, warp_group_base + 2);
          const unsigned int lane2_w =
              __shfl_sync(0xffffffffu, packed_line.w, warp_group_base + 2);
          unsigned int low0;
          unsigned int low1;
          unsigned int high;
          if (group_lane == 0) {
            ...
            low1 = packed_line.y;
            high = packed_line.z;
          } else if (group_lane == 1) {
            low0 = lane0_w;
            low1 = packed_line.x;
            high = packed_line.y;
          } else if (group_lane == 2) {
            low0 = lane1_z;
            low1 = lane1_w;
            high = packed_line.x;
          } else {
          ...
          const unsigned long long low =
              ...
              (static_cast<unsigned long long>(low1) << 32);
          int4 expanded;
          unsigned int* words = reinterpret_cast<unsigned int*>(&expanded);
  #pragma unroll
          for (int word = 0; word < 4; ++word) {
            const int value = word * 4;
            const unsigned int nibbles =
                static_cast<unsigned int>((low >> (value * 4)) & 0xffffu);
            const unsigned int high4 = (high >> value) & 0x0fu;
            words[word] =
                (nibbles & 0x000fu) |
                ((nibbles & 0x00f0u) << 4) |
                ((nibbles & 0x0f00u) << 8) |
          ...
          sh_b_stage[threads * i + threadIdx.x] = expanded;
        } else {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      if constexpr (has_act_order && !is_a_8bit) {
        ...
        scale_and_sub<a_type_id>(frag_b1, s2.y, frag_zp[j].y);
                           ...
                           !is_a_8bit) {
        // XRT's GGUF Q4_K adapter supplies a direct affine offset for the
        // float-zero-point specialization. Integer zero points retain the
        // upstream multiply-by-scale behavior.
        if (is_new_zp && !is_zp_float)
  ...
  auto write_result = [&](bool last) {
    if (xrt_f32_output) __syncthreads();
    int c_gl_stride = prob_n / 8;
    ...
    auto write = [&](int idx, float c0, float c1, FragS& s, FragS& b_bias) {
      ...
      if (xrt_f32_output) {
        if constexpr (m_block_size_8) {
          sh_xrt_f32[idx] = c0;
          sh_xrt_f32[idx + 8 * c_sh_stride] = c1;
        } else {
          sh_xrt_f32[2 * idx] = c0;
          sh_xrt_f32[2 * idx + 1] = c1;
        }
        return;
      }
      c_scalar_t2 res =
  ...
  #pragma unroll
    for (int i = 0;
      ...
      if (c_gl_wr < c_gl_wr_end) {
        if (xrt_f32_output) {
          float* C_f32 = reinterpret_cast<float*>(C);
  #pragma unroll
          for (int a = 0; a < 8; a++) {
            C_f32[8 * c_gl_wr + a] = sh_xrt_f32[8 * c_sh_rd + a];
          }
  ...
  #pragma unroll
      ...
      cp_async_wait<0>();
      // The FP32 epilogue keeps the Marlin tensor-core path live long enough
      // for racecheck to expose an upstream lifetime boundary: sh_b and sh_red
      // alias the same storage, so every thread must finish its last sh_b read
      // before any thread starts the block reduction writes.
      if (xrt_f32_output) __syncthreads();
