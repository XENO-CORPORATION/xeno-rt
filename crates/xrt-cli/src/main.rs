...
fn run_download(args: DownloadArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let model = if let Some(file) = args.file {
        ...
        hub.download_with_progress(&args.repo, &file, |progress| reporter(progress))?
    } else {
        ...
        hub.download_by_quantization(&args.repo, quantization, |progress| reporter(progress))?
    };
...
fn resolve_model_path(args: &GenerateArgs) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    let model = hub.download_with_progress(repo, file, |progress| reporter(progress))?;
    finish_download(
        &model.repo_id,
        &model.filename,
        model.path.as_path(),
